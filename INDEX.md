...
- [[开源项目分析/AI浏览器自动化/Stagehand-AI浏览器自动化框架|Stagehand — AI 浏览器自动化框架]] — Browserbase 的浏览器 agent SDK；v4 已完全脱离 Playwright（依赖层实测无 playwright），自研 CDP + 扩展，三原语 observe/act/extract，24,323 Stars，MIT
- [[开源项目分析/AI浏览器自动化/AI浏览器自动化赛道对比|AI 浏览器自动化赛道对比]] — **七家全景**（Playwright / Stagehand / browser-use / browser-harness / BrowserSkill / chrome-devtools-mcp / playwright-mcp）收敛为三个范式：确定性驱动 / SDK 编排 / 已登录真浏览器桥接；含关键分歧点与选型建议，附 qa-use 参考实现评估
- [[开源项目分析/AI浏览器自动化/BrowserSkill-复用真实登录态浏览器给Agent|BrowserSkill — 复用真实登录态浏览器给 Agent]] — 腾讯开源 bsk CLI + Chromium 扩展：Agent Window 沙箱 + 显式 tab borrow 归还协议 + request-help 人机接力，harness 无关（官方支持 Hermes Agent），4,259 Stars，MIT
