...
```typescript
options:
  - cwd // 当前工作目录，默认为当前工作目录，也可进行指定，但如果指定不存在，则报错 ENOENT
	- env // 环境变量
 	- uid // 用户表示
 	- gid // 进程群组标识
...
```
execFile 是 spawn 的一种衍生方法，本质还是调用的 spawn，exec是直接调用了execFile
```typescript
// 
...
```
### 
execFile和exec相差不多，只是execFile默认不会衍生shell，指定执行的命令而衍生新的进程
## fork
```typescript
...
```
execFileSync、execSync 为 spawnSync衍生
```typescript
// lib/child_process.js
